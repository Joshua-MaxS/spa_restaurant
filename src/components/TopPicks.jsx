	...
	return (
		<>
			...
			<div className='hidden lg:flex max-w-[1520px] m-auto py-2 px-2'>
				<Splide
					options={{ perPage: 4, gap: '.5rem', drag: 'free', arrows: false }}
				>
					{topPicks.map((item) => {
						return (
							<SplideSlide key={item.id}>
								<div className='rounded-3xl relative'>
									<div className='absolute w-full h-full bg-black/50 rounded-3xl text-white'>
										...
										<button className='px-3 py-1 rounded-lg bg-gray-950 mx-2 absolute bottom-4 hover:scale-110 duration-300 border-dotted text-gray-100 border-red-500'>
											Add to Cart
										</button>
									</div>
